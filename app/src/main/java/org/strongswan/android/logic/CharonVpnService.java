...
public class CharonVpnService extends VpnService implements Runnable {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null) {
            ...
            if (bundle != null) {
                ...
                if (profile != null) {
                    Log.e(TAG, "onStartCommand sharon profile != null");
                    String password = bundle.getString(VpnProfileDataSource.KEY_PASSWORD);
                    profile.setPassword(password);
                    Log.e(TAG, "onStartCommand sharon password " + password);
                    Log.e(TAG, "onStartCommand sharon user " + profile.getUsername());
                }
            }
    ...
    public void onDestroy() {
        ...
        setNextProfile(null);
        try {
            Log.e(TAG, "mConnectionHandler join");
        ...
        while (true) {
            synchronized (this) {
                try {
                    ...
                    boolean wasConnected = mCurrentProfile!=null;
                    Log.e(TAG, "was connected: " +wasConnected);
                    ...
                    if(wasConnected){
                        ...
                        boolean isNeedBreak = connectToNewServer();
                        if (isNeedBreak)
                            break;
                    }
    ...
    /*
    * Added this because we have problem with
    * internet if do reconnect immediately.
    * So we wait for one second to disconnect
    * finished.
     */
    public void startConnectionWithDelay(){
        ...
        handler.postDelayed(new Runnable() {
            ...
            public void run() {
                boolean isNeedBreak = connectToNewServer();
                if(isNeedBreak)
                    CharonVpnService.this.isNeedBreak = true;
            }
        }, 1000);
    }


    public boolean connectToNewServer(){
        if (mNextProfile == null) {
            ...
            if (initializeCharon(builder, mLogFile, mCurrentProfile.getVpnType().has(VpnTypeFeature.BYOD))) {
                ...
                //Log.e("connection.username", mCurrentProfile.getUsername());
                //Log.e("connection.password", mCurrentProfile.getPassword());
                //Log.e("connection.type", mCurrentProfile.getVpnType().getIdentifier());
                //Log.e("connection.server", mCurrentProfile.getGateway());
    ...
    private void stopCurrentConnection() {
        Log.e(TAG, "stopCurrentConnection");
        synchronized (this) {
            if (mCurrentProfile != null) {
                setState(State.DISCONNECTING);
                mIsDisconnecting = true;
                deinitializeCharon();
                Log.e(TAG, "charon stopped");
                mCurrentProfile = null;
            }
        }
    }
...
//				}
    ...
    private void setImcState(ImcState state) {
        Log.e(TAG, "setImcState " + state.name());
        synchronized (mServiceLock) {
            if (mService != null) {
    ...
    private void setErrorDisconnect(ErrorState error) {
        Log.e(TAG, "setErrorDisconnect " + error.name());
        synchronized (mServiceLock) {
            if (mService != null) {
                if (!mIsDisconnecting) {
                    mService.setError(error);
    ...
    public void startSimulate() {
        ...
        Runnable runnable = new Runnable() {
            @Override
            public void run() {
                updateStatus(STATE_CHILD_SA_DOWN);
            }
        };
