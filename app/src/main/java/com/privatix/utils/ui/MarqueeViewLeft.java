 ...
 */
public class MarqueeViewLeft extends LinearLayout {
    private static final int TEXTVIEW_VIRTUAL_WIDTH = 5000;
    private static final String TAG = MarqueeViewLeft.class.getSimpleName();
    /**
    ...
    private int textViewWidth = 2000;
    ...
    public MarqueeViewLeft(Context context) {
        super(context);
        init(context);
    }

    @SuppressWarnings({"UnusedDeclaration"})
    public MarqueeViewLeft(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public MarqueeViewLeft(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void extractAttributes(AttributeSet attrs) {
        ...
        TypedArray a = getContext().obtainStyledAttributes(attrs, com.privatix.R.styleable.MarqueeView);

        if (a == null) {
            return;
        }

        mSpeed = a.getInteger(com.privatix.R.styleable.MarqueeView_speed, DEFAULT_SPEED);
        mAnimationPause = a.getInteger(com.privatix.R.styleable.MarqueeView_pause, DEFAULT_ANIMATION_PAUSE);
        mAutoStart = a.getBoolean(com.privatix.R.styleable.MarqueeView_autoStart, false);
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (getChildCount() == 0 || getChildCount() > 1) {
            Log.e("MarqueeView", "MarqueeView must have exactly one child element.");
...
//        mScrollView.addView(mTextField);
    ...
    private void prepareAnimation() {
        ...
        mMarqueeNeeded = (mTextWidth - getMeasuredWidth()) > 300;
        if (!mMarqueeNeeded) {
            while (!mMarqueeNeeded) {
                String text = mTextField.getText().toString();
                String resultText = text + text;
                mTextField.setText(resultText);
                mTextWidth = mPaint.measureText(mTextField.getText().toString());
                //Log.e("mMarqueeNeeded", "mMarqueeNeeded");
                //Log.e("resultText", resultText);
                mMarqueeNeeded = (mTextWidth - getMeasuredWidth()) > 300;
                textViewWidth = (int) (textViewWidth * 1.5);
            }
        }
        /*String text = mTextField.getText().toString();
...
*/
        ...
        //Log.e("textDiff", (-mTextDifference) + "");
    ...
    private void expandTextView() {
        //Log.e("textViewWidth", textViewWidth+ " px");
